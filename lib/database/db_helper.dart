...
class DBHelper {
  ...
  */
  ...
  Future<int> insertMedication(Medication med) async {
    ...
    // Cloud Sync
    final insertedMed = med.copyWith(id: id);
    SyncService().pushMedication(insertedMed);
  ...
  Future<void> updateTakenStatus(int medicationId, bool isTaken) async {
    ...
    if (updated > 0) {
      ...
      // Cloud Sync
      final medData = result.first;
      final med = Medication.fromMap(medData).copyWith(isTaken: isTaken);
      SyncService().pushMedication(med);
    } else {
  ...
  Future<void> resetAllTaken() async {
    ...
    // Cloud Sync
    SyncService().resetAllTakenStatuses();
  }
  ...
  Future<void> deleteMedication(int id) async {
    ...
    // Cloud Sync
    SyncService().deleteMedication(id);
  }
